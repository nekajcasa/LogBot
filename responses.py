...
def rezerviraj_termin(message, user, dan, debug=False):
    ...
    try:
        ...
        if (termin):
            ...
            response = "Napaka#0:Napačen vnos, napiši '!help' za pomoč ali kontaktiraj admina."
    except:
        response = "Napaka#1: Napačen vnos, napiši '!help' za pomoč ali kontaktiraj admina."
...
class Dan:
    def __init__(self, datum):
        ...
        self.events = []
        self.uporaba = []
        self.db = GSDB.DB("gs_credentials.json")
...
class Event:
    def __init__(self, prihod, odhod, oseba, date):
        self.prihod = prihod
        self.odhod = odhod
        self.oseba = oseba
        self.datum = date
        t1 = datetime.datetime(date.year, date.month, date.day, prihod.hour, prihod.minute)
